...
/// View used to interact with vLLM using the Llama-Stack API
///
/// This view contains the code required to implement a text chat with a vLLM server that is implemented as a remote inference server provider for a Llama-Stack server.
/// Interaction with the Llama-Stack server is performed using the inference API that is part of the Llama-Stack specification.
struct LlamaStackChatView: View {
	...
	/// list of servers persisted by Swift Data that is used to select what server to interact with
