...
/// View used to interact with vLLM using the  [URL Loading system](https://developer.apple.com/documentation/foundation/httpurlresponse) in Apple Foundation technology.
///
...
/// Interaction with a  server is performed using Alamofire functionality.
struct FoundationChatView: View {
	...
	@StateObject private var chatManager: FoundationChatViewModel = .init()  // code for interacting with the vLLM server
